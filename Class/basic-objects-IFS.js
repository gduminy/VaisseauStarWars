...
function CubeAile(side) {
   ...
   function face(xyz, nrm) {
      ...
      for (i = 0; i < 4; i++) {
        ...
        texCoords.push(0,1,0,0,1,0,1,1);
 ...
 /**
  * Créé un modéle de triangle en 3D
  * @side the length of a side of the cube.  If not given, the value will be 1.
  */

function Triangle3D(side) {
   var s = (side || 1)/2;
   var coords = [];
   var normals = [];
   var texCoords = [];
   ...
   face( [-s*0.2,-s,s, s*0.2,-s,s, s*0.2,s*0.3,s, -s*0.2,s*0.3,s], [0,0,1] );
   face( [s*0.2,s*0.3,s, -s*0.2,s*0.3,s, -s*0.2,-s,-s, s*0.2,-s,-s], [0,0,-1] );
   //face( [-s,s,-s, -s,s,s, s,s,s, s,s,-s], [0,1,0] );
   face( [-s*0.2,-s,-s, s*0.2,-s,-s, s*0.2,-s,s, -s*0.2,-s,s], [0,-1,0] );
   face( [-s*0.2,s*0.3,s, -s*0.2,-s,s,-s*0.2,-s,-s,], [-1,0,0] );
   face( [s*0.2,s*0.3,s, s*0.2,-s,s, s*0.2,-s,-s,], [1,0,0] );
   return {
      vertexPositions: new Float32Array(coords),
      vertexNormals: new Float32Array(normals),
 ...
 /**
  * Créé un modéle de tétraédre 
  * @side the length of a side of the cube.  If not given, the value will be 1.
  */
function tétraédre(side) {
   var s = (side || 1)/2;
   var coords = [];
   var normals = [];
   var texCoords = [];
   ...
   function face(xyz) {
      ...
      A = vec3(xyz[6]-xyz[3], xyz[7]-xyz[4], xyz[8]-xyz[5]);
      B = vec3(xyz[0]-xyz[6], xyz[1]-xyz[7], xyz[2]-xyz[8]);
      AB = normalize(cross(A,B));
      console.log(AB);
      for (i = 0; i < 4; i++) {
         normals.push(AB[0],AB[1],AB[2]);
      }
      texCoords.push(0,0,1,0,1,1,0,1);
      indices.push(start,start+1,start+2);
   }
   face([-s,-s,-s, s,-s,s, -s,s,s]);
   face([s,s,-s, -s,s,s,   s,-s,s]);
   face( [  -s,-s,-s,-s,s,s,  s,s,-s]);
   face( [s,s,-s,  s,-s,s,  -s,-s,-s ]);
   return {
